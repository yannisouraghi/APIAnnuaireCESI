...
public class SiteController : ControllerBase
{
    ...
    public async Task<ActionResult> DeleteSite(int id)
    {
        try
        {
            ...
            var employees = await _context.Employees.Where(e => e.SiteId == id).ToListAsync();
            if (employees.Any())
            {
                return BadRequest($"Le site {siteToDelete.City} ne peut pas être supprimé car il est asscoié à {employees.Count} employés.");
            }
